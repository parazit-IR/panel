...
@Component
public class SupportTelegramCommandHandler implements TelegramCommandHandler {

    private final TelegramMainMenuHandler mainMenuHandler;

    public SupportTelegramCommandHandler(TelegramMainMenuHandler mainMenuHandler) {
    ...
    public TelegramCommand command() {
        return TelegramCommand.SUPPORT;
    }

    @Override
    public TelegramResponsePlan handle(TelegramInteractionContext context) {
        return mainMenuHandler.handle(context, TelegramMainMenuAction.SHOW_SUPPORT);
    }
}
